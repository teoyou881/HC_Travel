...
function NavItem({mobile}) {
  ...
  const userType = useSelector((state) => state.user.userData?.userType); // 사용자 타입 가져오기
  ...
  return (
      <ul
        ...
        {routes.map(({to, name, auth, icon, userType}) => {
          // 로그인 상태 확인
          if (isAuth !== auth) return null;

          if (name === 'upload' && userType === 'admin') return null;
            ...
            return (
                <li
                  ...
                  <Link to={to}>
                    {icon}
                    <span
                        className="absolute top-0 inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -right-3">
                                    {cart.length}
                                </span>
                  </Link>
                </li>
